...
-- Get user who did not login in last five months
SELECT U.USER_ID
FROM USERS                 U
...
-- Get users that logged in daily from day they first login
SELECT *
FROM
(
    SELECT USER_ID
         , MIN(CAST(LOGIN_TIMESTAMP AS DATE))                               AS FIRST_LOGIN
         , DATEDIFF(DAY, MIN(CAST(LOGIN_TIMESTAMP AS DATE)), GETDATE()) + 1 AS LOGIN_REQUIRED
         , COUNT(DISTINCT (CAST(LOGIN_TIMESTAMP AS DATE)))                  AS TOTAL_LOGIN
    FROM LOGINS
    GROUP BY USER_ID
) DATA
WHERE LOGIN_REQUIRED = TOTAL_LOGIN
-- Get the dates where there was no login
WITH ALL_DATE
AS (SELECT
        (
            SELECT MIN(CAST(LOGIN_TIMESTAMP AS DATE)) FROM LOGINS
        ) AS MIN_DATE
    UNION ALL
    SELECT DATEADD(DAY, 1, MIN_DATE) AS MIN_DATE
    FROM ALL_DATE
    WHERE MIN_DATE <= CAST(GETDATE() AS DATE)
   )
   , LOGIN_DATA
AS (SELECT CAST(LOGIN_TIMESTAMP AS DATE) AS LOGIN_DATE
...
(
    SELECT DISTINCT
        A.MIN_DATE
    FROM ALL_DATE                  A
        LEFT OUTER JOIN LOGIN_DATA L
            ON L.LOGIN_DATE = A.MIN_DATE
